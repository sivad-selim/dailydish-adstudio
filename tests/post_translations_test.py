...
spec = importlib.util.spec_from_file_location('translations', Path(__file__).parents[1] / 'scripts/post-translations.py')
...
class TranslationTests(unittest.TestCase):
    def test_gallery_selects_each_owned_page_in_order(self):
        pages = [message('a'), message('b')]
        posts = [{'id': 'p', 'data': {'folderId': 'f', 'pageIds': ['b', 'a']}}]
        folders = [{'id': 'f', 'data': {'name': 'Collection'}}]
        self.assertEqual(m.select_pages(pages, posts, folders, 'Collection'), list(reversed(pages)))
        with self.assertRaises(ValueError):
            m.select_pages(pages[:1], posts, folders, 'Collection')

    def test_ambiguous_name_fails_and_br_maps_to_pt(self):
        with self.assertRaises(ValueError):
            m.resolve([message(), message('c2')], 'Titre', m.title)
        self.assertEqual(m.language_list('EN,BR,pt'), ['en', 'pt'])

    def test_optional_empty_description_is_not_missing(self):
        plan = m.prepare([message()], ['en', 'pt'], 'fr')
        self.assertEqual(plan['pages'][0]['missing'], {'en': ['title'], 'pt': ['title']})

    def test_field_mask_preserves_other_data_and_uses_atomic_precondition(self):
        row = message()
        plan = m.prepare([row], ['en'], 'fr')
        plan['pages'][0]['proposed'] = {'en': {'title': 'Title'}}
    ...
    def test_overwrite_and_source_protection(self):
        ...
        plan['pages'][0]['proposed'] = {'en': {'description': 'Changed'}}
        with self.assertRaises(ValueError):
            m.build_writes(plan, {'c1': row})
        self.assertEqual(len(m.build_writes(plan, {'c1': row}, True)), 1)
        plan['pages'][0]['proposed'] = {'fr': {'title': 'Changed'}}
        with self.assertRaises(ValueError):
    ...
    def test_unrequested_language_and_empty_text_rejected(self):
        ...
        for proposed in [{'pt': {'title': 'Titulo'}}, {'en': {'title': ''}}, {'en': {'folderId': 'other'}}]:
            plan['pages'][0]['proposed'] = proposed
            with self.assertRaises(ValueError):
                m.build_writes(plan, {'c1': row})
